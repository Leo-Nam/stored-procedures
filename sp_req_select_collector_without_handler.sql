...
BEGIN
	...
	IF ROW_COUNT() = 1 THEN
		...
		IF ROW_COUNT() = 1 THEN
			IF IN_RANK = 1 THEN
				SELECT SECOND_PLACE INTO @SECOND_PLACE FROM SITE_WSTE_DISPOSAL_ORDER WHERE ID = IN_DISPOSER_ORDER_ID;
				IF @SECOND_PLACE IS NOT NULL THEN
					UPDATE COLLECTOR_BIDDING 
					SET MAX_DECISION_AT = ADDTIME(@SELECTED_AT, CONCAT(CAST(@max_decision_duration AS UNSIGNED)*2, ':00:00'))
            ...
            ELSE
				IF @TRANSACTION_COUNT = 1 THEN
					...
					SET COLLECT_ASK_END_AT = IN_DISCHARGED_END_AT 
					WHERE 
						DISPOSAL_ORDER_ID = IN_DISPOSER_ORDER_ID AND 
						COLLECTOR_BIDDING_ID = IN_COLLECTOR_BIDDING_ID;
