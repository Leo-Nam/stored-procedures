...
*/
	...
	/*등록을 요청하는 사용자의 USER_ID가 이미 등록되어 있는 경우에는 @USER_EXISTS = 1, 
    그렇지 않은 경우에는 @USER_EXISTS = 0이 됨*/ 		
	IF @rtn_val = 0 THEN
    /*사용자가 존재하는 경우*/
		...
		/*사이트가 정상(개인사용자는 제외됨)적인 경우*/
			...
			/*사이트가 배출자로부터 최종 낙찰자로 선정된 경우*/
				...
				IF ROW_COUNT() = 1 THEN
				/*데이타베이스 입력에 성공한 경우*/
					SELECT BIDDING_RANK, COLLECTOR_ID 
                    INTO @BIDDING_RANK, @COLLECTOR_ID 
                    FROM COLLECTOR_BIDDING 
                    WHERE ID = IN_COLLECT_BIDDING_ID;
                    
					IF @BIDDING_RANK = 1 THEN
						CALL sp_req_policy_direction(
						/*수거자가 배출자의 최종입찰선정에 응답을 할 수 있는 최대의 시간으로서 
                        배출자의 최종낙찰자선정일로부터의 기간을 반환받는다(단위:시간)*/
						...
						IF ROW_COUNT() = 1 THEN
							...
							/*최종결정을 수락한 경우에는 CLCT_TRMT_TRANSACTION에 
                            이미 생성되어 있는 작업을 UPDATE한다.*/
								UPDATE SITE_WSTE_DISPOSAL_ORDER 
								SET 
								...
								IF ROW_COUNT() = 1 THEN
								/*WSTE_CLCT_TRMT_TRANSACTION에 이미 생성되어 있는 작업사항 중 
                                수거자결정 내용 변경에 성공한 경우*/                                      
									SET @rtn_val 		= 0;
									SET @msg_txt 		= CONCAT('Success2: ', @BIDDING_RANK);
									SET @json_data 		= NULL;
								ELSE
								/*WSTE_CLCT_TRMT_TRANSACTION에 이미 생성되어 있는 작업사항 중 
                                수거자결정 내용 변경에 실패한 경우 예외처리한다.*/
									SET @rtn_val 		= 24102;
							...
							/*최종결정을 거절한 경우*/	
								SET @rtn_val 		= 0;
								SET @msg_txt 		= CONCAT('Success1: ', @BIDDING_RANK);
								SET @json_data 		= NULL;
							END IF;
						ELSE
							SET @rtn_val 		= 24104;
                    ...
                    ELSE
						IF @BIDDING_RANK = 2 THEN
							...
							IF ROW_COUNT() = 1 THEN
                                ...
                                이미 생성되어 있는 작업을 UPDATE한다.1*/
                                    ...
                                    수거자결정 내용 변경에 성공한 경우*/
										SET @rtn_val 		= 0;
										SET @msg_txt 		= CONCAT('Success4: ', @BIDDING_RANK);
										SET @json_data 		= NULL;
									ELSE
									/*WSTE_CLCT_TRMT_TRANSACTION에 이미 생성되어 있는 작업사항 중 
                                    수거자결정 내용 변경에 실패한 경우 예외처리한다.*/
								...
								ELSE
									SET @rtn_val 		= 0;
									SET @msg_txt 		= CONCAT('Success5: ', @BIDDING_RANK);
									SET @json_data 		= NULL;
								END IF;
							ELSE
								SET @rtn_val 		= 24106;
						...
						ELSE
							SET @rtn_val 		= 24105;
							SET @msg_txt 		= 'Failed to change emitter record1';
							SET @json_data 		= NULL;
							SIGNAL SQLSTATE '23000';
						END IF;
                    END IF;
