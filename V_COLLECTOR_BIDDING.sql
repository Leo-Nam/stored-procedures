...
VIEW `chiumdev_2`.`V_COLLECTOR_BIDDING` AS
    SELECT 
        ...
        `A`.`REJECT_DECISION` AS `COLLECTOR_REJECT_DECISION`,
                ...
                IF((`B`.`VISIT_END_AT` > NOW()),
                    IF((`A`.`RESPONSE_VISIT` IS NOT NULL),
                        IF((`A`.`RESPONSE_VISIT` = TRUE),
                            IF((`A`.`CANCEL_VISIT` IS NOT NULL),
                                IF((`A`.`CANCEL_VISIT` = FALSE),
                                    IF((`B`.`VISIT_EARLY_CLOSING` = FALSE),
                                        201,
                                        IF((`B`.`BIDDING_END_AT` > NOW()),
                                            IF((`A`.`CANCEL_BIDDING` = FALSE),
                                            ...
                                            IF((`A`.`SELECTED` IS NOT NULL),
                                                IF((`A`.`SELECTED` = TRUE),
                                                    IF((`A`.`REJECT_DECISION` IS NULL),
                                                        208,
                                                        IF((`A`.`REJECT_DECISION` = TRUE),
                                                            210,
                                                            IF((`B`.`CLOSE_AT` >= NOW()), 211, 212))),
                                                    213),
                                                209))),
                                    202),
                                217),
