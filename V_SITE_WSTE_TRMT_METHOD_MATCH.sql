...
VIEW `chiumdev_2`.`V_SITE_WSTE_TRMT_METHOD_MATCH` AS
    SELECT 
        `A`.`ID` AS `ID`,
        `E`.`ID` AS `COMP_ID`,
    ...
    FROM
        (((((((`chiumdev_2`.`SITE_WSTE_TRMT_METHOD_MATCH` `A`
        ...
        LEFT JOIN `chiumdev_2`.`COMP_SITE` `H` ON ((`A`.`SITE_ID` = `H`.`ID`)))
        LEFT JOIN `chiumdev_2`.`COMPANY` `E` ON ((`H`.`COMP_ID` = `E`.`ID`)))
